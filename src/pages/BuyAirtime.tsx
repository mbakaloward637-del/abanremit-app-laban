...
const BuyAirtime = () => {
  ...
  const handlePurchase = async () => {
    if (!user || !amount || !selectedNetwork) return;
    setProcessing(true);
    try {
      await api.airtime.purchase({ amount: Number(amount), phone, network: selectedNetwork });
      await refreshUser();
      setDone(true);
      toast.success(`${user.currency} ${amount} airtime sent to ${phone}`);
    } catch (err: any) {
      ...
      <div className="px-5 pt-6">
          ...
          <div className="space-y-3">
            ...
            {networks.map((n) => (
              <button key={n.id} onClick={() => { setSelectedNetwork(n.id); setStep(2); }}
                ...
                <div className={`flex h-12 w-12 items-center justify-center rounded-xl ${n.color}`}><Smartphone size={22} className={n.iconColor} /></div>
                <div className="text-left"><p className="text-sm font-semibold text-foreground">{n.name}</p><p className="text-xs text-muted-foreground">Mobile Airtime</p></div>
              </button>
            ))}
          </div>
        )}
        {step === 2 && (
          <div className="space-y-4">
            <div><label className="label-text">Phone Number</label><input value={phone} onChange={(e) => setPhone(e.target.value)} className="input-field" placeholder="+254 7XX XXX XXX" /></div>
            <div>
              <label className="label-text">Amount ({user?.currency})</label>
              <div className="grid grid-cols-4 gap-2 mb-3">
                {amounts.map((a) => (
                  <button key={a} onClick={() => setAmount(String(a))}
                    className={`rounded-xl border py-3 text-xs font-semibold transition-all ${amount === String(a) ? "border-primary bg-primary/10 text-primary" : "border-border text-foreground hover:border-primary/30"}`}>{a.toLocaleString()}</button>
                ))}
              </div>
        ...
        {step === 3 && (
          <div className="space-y-4">
            ...
            <div><label className="label-text">Wallet PIN</label><input type="password" maxLength={6} className="input-field text-center text-xl tracking-[0.5em]" placeholder="••••" /></div>
