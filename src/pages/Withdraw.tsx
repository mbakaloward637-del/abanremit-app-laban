...
const Withdraw = () => {
  ...
  const handleWithdraw = async () => {
    ...
    try {
      ...
      await api.transactions.withdraw({ amount: Number(amount), method, destination: dest, pin });
      await refreshUser();
      setDone(true);
          ...
          <div className="flex rounded-xl border border-border p-1 bg-secondary">
            {([{ id: "bank" as const, icon: Building2, label: "Bank" }, { id: "mobile" as const, icon: Smartphone, label: "Mobile Money" }]).map((m) => (
              <button key={m.id} onClick={() => setMethod(m.id)}
                className={`flex-1 flex items-center justify-center gap-2 rounded-lg py-3 text-xs font-medium transition-all ${method === m.id ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}>
